...
function dashboard(){
  ...
  const activePos=pos.filter(record=>!["Completed","Cancelled"].includes(record[6])).length;
  const completedPos=pos.filter(record=>record[6]==="Completed").length;
  const pendingIars=iars.filter(record=>record[6]!=="Completed").length;
  const completedIars=iars.filter(record=>record[6]==="Completed").length;
  const expendableItems=masters.Items.filter(record=>record[6]==="Expendable").length;
  const semiUnits=propertyUnits.filter(record=>record.classification==="Semi-Expendable").length;
  const capitalUnits=propertyUnits.filter(record=>record.classification==="Capital Outlay").length;
  const issuedUnits=risRecords.filter(record=>record.status==="Completed").flatMap(record=>record.lines||[]).reduce((sum,line)=>sum+Number(line.qty||0),0);
  return `<section class="welcome-row"><div><h2>Good morning, Supply Officer</h2><p>Here’s the current status of your inventory and property records.</p></div><div class="period-chip">Reporting period<strong>${period}</strong></div></section>
  <section class="metrics-grid">${metric("Active Purchase Orders",activePos,"Based on saved records","blue")}${metric("Completed POs",completedPos,"Based on saved records","green")}${metric("Pending IARs",pendingIars,"Awaiting completion","amber")}${metric("Completed IARs",completedIars,"Based on saved records","teal")}${metric("Expendable Item Types",expendableItems,"From item master","indigo")}${metric("Semi-Expendable Units",semiUnits,"From property records","violet")}${metric("Capital Outlay Units",capitalUnits,"From property records","navy")}${metric("Items Issued",issuedUnits,"From completed RIS","sky")}</section>
  <section class="dashboard-grid"><article class="panel span-2"><div class="panel-heading"><div><h3>Monthly inventory issuance</h3><p>Issued quantities will appear after completed RIS transactions.</p></div></div><div class="empty-state"><span>CH</span><h3>No issuance data yet</h3><p>Complete an RIS to begin building this chart.</p></div></article>
  <article class="panel"><div class="panel-heading"><div><h3>Inventory composition</h3><p>Based on saved inventory and property records</p></div></div><div class="empty-state"><span>IN</span><h3>No inventory data yet</h3><p>Complete an IAR to add inventory.</p></div></article>
  <article class="panel span-2"><div class="panel-heading"><div><h3>Recent purchase orders</h3><p>Latest procurement activity</p></div><button class="text-button" data-go="Purchase Orders">View all →</button></div>${pos.length?panelTable(pos.slice(0,4)):`<div class="empty-state"><span>PO</span><h3>No purchase orders yet</h3><p>Create your first purchase order to begin.</p></div>`}</article>
  <article class="panel"><div class="panel-heading"><div><h3>Low-stock items</h3><p>At or below reorder level</p></div><span class="count-pill">0 items</span></div><div class="empty-state"><span>ST</span><h3>No stock records yet</h3><p>Stock alerts will appear after inventory is received.</p></div></article></section>`;
}
...
function enhancedAdmin(){
  ...
  const addButton=adminTab==="Departments"||adminTab==="Users"||adminTab==="Activity Log"?"":`<button class="primary-button" id="add-master">＋ Add ${singular}</button>`;
...
function adminContent(){
  ...
  if(adminTab==="System Settings")return `<section class="settings-grid"><article class="panel"><div class="panel-heading"><div><h3>Accounting and reporting settings</h3><p>Configurable values for office review and validation.</p></div></div><div class="setting-form"><label>Capitalization threshold<span class="money-input"><b>₱</b><input id="threshold" type="number" value="50000"></span></label><div class="form-grid"><label>Office name<input placeholder="Enter office name"></label><label>Office address<input placeholder="Enter office address"></label><label>Default fund source<select><option>Regular Fund 01</option></select></label><label>Current reporting year<input type="number" value="${new Date().getFullYear()}"></label><label class="wide">Permanent delivery location<input value="3rd Floor Esquina Dos Bldg, J.C. Aquino Ave, Butuan City" readonly></label></div><button class="primary-button" id="save-setting">Save settings</button></div></article><article class="panel"><div class="panel-heading"><div><h3>Default report signatories</h3><p>Names and titles remain configurable.</p></div></div><div class="setting-form"><div class="form-grid"><label>Prepared by<input placeholder="Name and position"></label><label>Certified by<input placeholder="Name and position"></label><label>Approved by<input placeholder="Name and position"></label></div><div class="notice-note">These values are administrative defaults, not electronic signatures.</div></div></article></section>`;
  const compactTables={
...
function bindEnhanced(){
  ...
  document.querySelector("#save-setting")?.addEventListener("click",e=>{e.target.textContent="✓ Settings saved";showToast("System settings updated. Future classifications will use the new values.");setTimeout(()=>e.target.textContent="Save settings",1800)});
