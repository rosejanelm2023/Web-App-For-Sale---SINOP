...
let stickerSelectedKeys=[];
let stickerSearch="";
let stickerClassification="All property";
const stickerFields={propertyNumber:true,item:true,brand:true,model:true,serial:true,cost:true,date:true,condition:true,endUser:true,inventoryDate:true,agencyRepresentative:true,coaRepresentative:true};
const stickerSettings={inventoryDate:localISODate(),agencyRepresentative:"",coaRepresentative:""};
...
function addYearsToIso(value,years){
...
function renewalBaseIso(unit){return normalizeDatePickerValue(unit.lastRenewalIsoDate||unit.isoDate||unit.acceptedDate||"")}
function renewalDueIso(unit){return unit.renewalDueIsoDate||addYearsToIso(renewalBaseIso(unit),3)}
function renewalDueProperties(){
  const today=localISODate();
  return propertyUnits.map((unit,index)=>({unit,index,due:renewalDueIso(unit)})).filter(entry=>["Semi-Expendable","Capital Outlay"].includes(entry.unit.classification)&&entry.unit.status!=="Disposed"&&entry.due&&entry.due<=today).sort((a,b)=>a.due.localeCompare(b.due)||String(a.unit.item).localeCompare(String(b.unit.item)));
}
function renewalDateLabel(value){const normalized=normalizeDatePickerValue(value);if(!normalized)return "—";return new Date(`${normalized}T00:00:00`).toLocaleDateString("en-PH",{month:"short",day:"numeric",year:"numeric"})}
function ensureRenewalTestProperty(){
...
function renewalAlertMarkup(){
  ...
  return `<section class="panel renewal-alert"><div class="panel-heading"><div><h3>Property accountability renewal</h3><p>PAR renewal is due every three years or when custodianship changes. Semi-Expendable ICS records follow the same agency review cycle here.</p></div><span class="renewal-count">${due.length} due</span></div><div class="renewal-list">${due.map(({unit,index,due:dueDate})=>{const documentNumber=unit.classification==="Semi-Expendable"?unit.icsNumber:unit.parNumber;return `<article><span class="renewal-warning">!</span><div><strong>${escapeFormValue(unit.item)}</strong><small>${escapeFormValue(unit.number||unit.inventoryNumber||"Identifier pending")} · ${escapeFormValue(unit.classification)}</small><p>Current ${unit.classification==="Semi-Expendable"?"ICS":"PAR"}: <b>${escapeFormValue(documentNumber||"Not assigned")}</b> · Due ${renewalDateLabel(dueDate)}</p></div><button class="primary-button" data-renew-property="${index}">Renew now</button></article>`}).join("")}</div></section>`;
}
...
const stickerFieldChoices=[["propertyNumber","Property / Inventory No."],["item","Item Description"],["brand","Brand"],["model","Model"],["serial","Serial No."],["cost","Acquisition Cost"],["date","Date Acquired"],["condition","Condition / Remarks"],["endUser","Location / End-User"],["inventoryDate","Inventory Date"],["agencyRepresentative","Agency Representative"],["coaRepresentative","COA Representative"]];
function stickerPropertyKey(unit,index){return String(unit.dbId||unit.inventoryNumber||unit.number||index)}
function stickerEligibleEntries(){return propertyUnits.map((unit,index)=>({unit,index,key:stickerPropertyKey(unit,index)})).filter(({unit})=>["Semi-Expendable","Capital Outlay"].includes(unit.classification)).sort((a,b)=>String(a.unit.item).localeCompare(String(b.unit.item),undefined,{sensitivity:"base",numeric:true})||String(a.unit.number||a.unit.inventoryNumber||"").localeCompare(String(b.unit.number||b.unit.inventoryNumber||""),undefined,{numeric:true}))}
function stickerFilteredEntries(){const query=stickerSearch.trim().toLowerCase();return stickerEligibleEntries().filter(({unit})=>(stickerClassification==="All property"||unit.classification===stickerClassification)&&(!query||[unit.item,unit.description,unit.number,unit.inventoryNumber,unit.brand,unit.model,unit.serial,unit.employee].join(" ").toLowerCase().includes(query)))}
function stickerDetailRows(unit){
  const number=unit.classification==="Semi-Expendable"?(unit.inventoryNumber||unit.number):(unit.number||unit.inventoryNumber);
  const fields={propertyNumber:["PROPERTY / INVENTORY NO.",number||"—"],item:["ITEM DESCRIPTION",[unit.item,unit.description&&unit.description!==unit.item?unit.description:""].filter(Boolean).join(" — ")||"—"],brand:["BRAND",unit.brand||"N/A"],model:["MODEL",unit.model||"N/A"],serial:["SERIAL NO.",unit.serial||"N/A"],cost:["ACQUISITION COST",Number(unit.cost||0).toLocaleString("en-PH",{style:"currency",currency:"PHP",minimumFractionDigits:2,maximumFractionDigits:2})],date:["DATE ACQUIRED",unit.date||renewalDateLabel(unit.isoDate)],condition:["CONDITION / REMARKS",[unit.condition,unit.otherInfo].filter(Boolean).join(" · ")||"—"],endUser:["LOCATION / END-USER",[unit.location||unit.office,unit.employee].filter(Boolean).join(" / ")||"—"],inventoryDate:["INVENTORY DATE",renewalDateLabel(stickerSettings.inventoryDate)],agencyRepresentative:["AGENCY REPRESENTATIVE",stickerSettings.agencyRepresentative||"________________"],coaRepresentative:["COA REPRESENTATIVE",stickerSettings.coaRepresentative||"________________"]};
  return stickerFieldChoices.filter(([key])=>stickerFields[key]&&!['agencyRepresentative','coaRepresentative'].includes(key)).map(([key])=>`<div class="inventory-sticker-row"><b>${fields[key][0]}</b><span>${escapeFormValue(fields[key][1])}</span></div>`).join("");
}
function inventoryStickerCard(entry){
  const tenant=readTenantSettings();const unit=entry.unit;
  const representatives=stickerFields.agencyRepresentative||stickerFields.coaRepresentative?`<footer>${stickerFields.agencyRepresentative?`<div><b>AGENCY REPRESENTATIVE</b><span>${escapeFormValue(stickerSettings.agencyRepresentative||"________________")}</span></div>`:""}${stickerFields.coaRepresentative?`<div><b>COA REPRESENTATIVE</b><span>${escapeFormValue(stickerSettings.coaRepresentative||"________________")}</span></div>`:""}</footer>`:"";
  return `<article class="inventory-sticker"><div class="inventory-sticker-details">${stickerDetailRows(unit)}</div><aside><canvas width="150" height="150" data-sticker-qr="${escapeFormValue(propertyQrPayload(unit,entry.index))}" aria-label="QR code for ${escapeFormValue(unit.item)}"></canvas>${tenant.logoPreview?`<img src="${escapeFormValue(tenant.logoPreview)}" alt="${escapeFormValue(tenant.agencyName)} logo">`:`<strong>${escapeFormValue((tenant.agencyName||"S").slice(0,2).toUpperCase())}</strong>`}<small>${escapeFormValue(tenant.agencyName||"Your Agency")}</small>${unit.isTest?`<em>TEST PROPERTY</em>`:""}</aside>${representatives}</article>`;
}
function inventoryStickerView(){
  const entries=stickerEligibleEntries();stickerSelectedKeys=stickerSelectedKeys.filter(key=>entries.some(entry=>entry.key===key));const selected=entries.filter(entry=>stickerSelectedKeys.includes(entry.key));const pages=Array.from({length:Math.ceil(selected.length/12)},(_,page)=>selected.slice(page*12,page*12+12));
  const propertyRows=stickerFilteredEntries().map(entry=>`<label class="sticker-property-option"><input type="checkbox" data-sticker-property="${escapeFormValue(entry.key)}" ${stickerSelectedKeys.includes(entry.key)?"checked":""}><span><strong>${escapeFormValue(entry.unit.item)}</strong><small>${escapeFormValue(entry.unit.number||entry.unit.inventoryNumber||"Identifier pending")} · ${escapeFormValue(entry.unit.classification)}</small></span>${entry.unit.isTest?`<b>TEST</b>`:""}</label>`).join("");
  return `<section class="page-heading no-print"><div><h2>Inventory Sticker</h2><p>Choose the property details to include, then print twelve side-by-side stickers per legal-size sheet.</p></div><button class="primary-button" id="print-inventory-stickers" ${selected.length?"":"disabled"}>Print ${selected.length} sticker${selected.length===1?"":"s"}</button></section><section class="sticker-builder no-print"><article class="panel sticker-config"><div class="panel-heading"><div><h3>Sticker content</h3><p>Check only the details your agency wants printed.</p></div><span class="count-pill">${stickerFieldChoices.filter(([key])=>stickerFields[key]).length} fields</span></div><div class="sticker-field-grid">${stickerFieldChoices.map(([key,label])=>`<label><input type="checkbox" data-sticker-field="${key}" ${stickerFields[key]?"checked":""}> ${label}</label>`).join("")}</div><div class="sticker-extra-fields"><label>Inventory date<input type="date" data-sticker-setting="inventoryDate" value="${escapeFormValue(stickerSettings.inventoryDate)}"></label><label>Agency representative<input data-sticker-setting="agencyRepresentative" value="${escapeFormValue(stickerSettings.agencyRepresentative)}" placeholder="Optional printed name"></label><label>COA representative<input data-sticker-setting="coaRepresentative" value="${escapeFormValue(stickerSettings.coaRepresentative)}" placeholder="Optional printed name"></label></div></article><article class="panel sticker-property-picker"><div class="panel-heading"><div><h3>Select property units</h3><p>Semi-Expendable and Capital Outlay only.</p></div><span class="count-pill">${selected.length} selected</span></div><div class="sticker-picker-toolbar"><label class="search">⌕<input id="sticker-search" value="${escapeFormValue(stickerSearch)}" placeholder="Search item or property number…"></label><select id="sticker-classification"><option ${stickerClassification==="All property"?"selected":""}>All property</option><option ${stickerClassification==="Semi-Expendable"?"selected":""}>Semi-Expendable</option><option ${stickerClassification==="Capital Outlay"?"selected":""}>Capital Outlay</option></select><button class="secondary-button" id="select-filtered-stickers">Select filtered</button><button class="secondary-button" id="clear-stickers">Clear</button></div><div class="sticker-property-list" id="sticker-property-list">${propertyRows||`<div class="empty-state compact"><h3>No matching property</h3><p>Change the search or classification.</p></div>`}</div></article></section>${selected.length?`<section class="sticker-preview-heading no-print"><div><h3>Print preview</h3><p>Legal-size portrait · two columns × six rows · twelve stickers per sheet</p></div><span>${pages.length} page${pages.length===1?"":"s"}</span></section>${pages.map((page,pageIndex)=>`<section class="inventory-sticker-sheet" data-sticker-page="${pageIndex+1}">${page.map(inventoryStickerCard).join("")}</section>`).join("")}`:`<section class="panel no-print"><div class="empty-state"><span>QR</span><h3>No stickers selected</h3><p>Select one or more property units to prepare the legal-size sticker sheet.</p></div></section>`}`;
}
function renderInventoryStickerQrCodes(){document.querySelectorAll("[data-sticker-qr]").forEach(canvas=>window.QRCode?.toCanvas(canvas,canvas.dataset.stickerQr,{width:150,margin:1,errorCorrectionLevel:"M",color:{dark:"#000000",light:"#FFFFFF"}}))}
...
function propertyQrPayload(property,index){
  ...
  return `sinop://property/${encodeURIComponent(permanentId)}/renewal/${Number(property.qrVersion||1)}`;
}
...
function nextRenewalIcsNumber(dateValue=localISODate()){
  const date=new Date(`${normalizeDatePickerValue(dateValue)||localISODate()}T00:00:00`);const year=date.getFullYear();const month=String(date.getMonth()+1).padStart(2,"0");
  const highest=propertyUnits.reduce((maximum,unit)=>{const match=String(unit.icsNumber||"").toUpperCase().match(/^(\d{4})-(\d{2})-(\d{4}|\d{3}[A-Z])$/);if(!match||Number(match[1])!==year)return maximum;const numeric=Number(match[3].replace(/[A-Z]$/,""));return Math.max(maximum,numeric||0)},0);
  return `${year}-${month}-${String(highest+1).padStart(4,"0")}`;
}

function openPropertyRenewal(index){
  const property=propertyUnits[index];if(!property)return;
  if(window.inventoryAccess&&!window.inventoryAccess.canWrite){showToast("Your account does not have permission to renew property accountability.","error");return}
  const semi=property.classification==="Semi-Expendable";const documentLabel=semi?"ICS":"PAR";const suggested=semi?nextRenewalIcsNumber():nextParNumber();const oldDocument=semi?property.icsNumber:property.parNumber;
  const currentReceiver=String(property.employee||"");
  const receiverExists=masters.Employees.some(employee=>String(employee[1]||"")===currentReceiver);
  const currentReceiverOption=currentReceiver&&!receiverExists?`<option value="${escapeFormValue(currentReceiver)}">${escapeFormValue(currentReceiver)} (Current receiver)</option>`:"";
  document.querySelector("#modal").innerHTML=`<div class="modal-backdrop"><form class="drawer compact-drawer" id="property-renewal-form"><div class="drawer-head"><div><p>Three-year accountability review</p><h2>Renew ${documentLabel}</h2></div><button type="button" id="close">×</button></div><div class="drawer-body"><div class="source-card"><span>${documentLabel}</span><div><strong>${escapeFormValue(property.item)}</strong><p>${escapeFormValue(property.number||property.inventoryNumber||"Identifier pending")} · Current ${documentLabel}: ${escapeFormValue(oldDocument||"Not assigned")}</p></div></div><div class="renewal-scope-note"><strong>Only these values will change:</strong><span>Receiver, ${documentLabel} number, and QR renewal version. The acquisition and property history remain unchanged.</span></div><div class="form-grid"><label>${documentLabel} No.<input name="documentNumber" value="${escapeFormValue(suggested)}" pattern="${semi?"[0-9]{4}-[0-9]{2}-([0-9]{4}|[0-9]{3}[A-Za-z])":"[0-9]{4}-([0-9]{4}|[0-9]{3}[A-Za-z])"}" required><small class="field-hint">Pre-populated and editable. Letter suffixes are accepted.</small></label><label>New receiver<select name="employee" id="renewal-receiver" required><option value="">Select employee</option>${currentReceiverOption}${propertyEmployeeOptions("")}</select></label><label class="wide renewal-same-receiver"><input type="checkbox" id="renewal-same-receiver" ${currentReceiver?"":'disabled title="No current receiver is assigned"'}><span>Same as current receiver${currentReceiver?`: ${escapeFormValue(currentReceiver)}`:" (no current receiver assigned)"}</span></label></div><div class="renewal-preview"><div><span>Previous receiver</span><strong>${escapeFormValue(property.employee||"Unassigned")}</strong></div><div><span>Next renewal due</span><strong>${renewalDateLabel(addYearsToIso(localISODate(),3))}</strong></div><div><span>New QR version</span><strong>Version ${Number(property.qrVersion||1)+1}</strong></div></div><p class="field-error" id="renewal-error"></p></div><div class="drawer-foot"><button type="button" class="secondary-button" id="cancel">Cancel</button><button type="submit" class="primary-button">Renew accountability</button></div></form></div>`;
  ...
  const sameReceiver=document.querySelector("#renewal-same-receiver");const receiverSelect=document.querySelector("#renewal-receiver");
  sameReceiver?.addEventListener("change",()=>{if(sameReceiver.checked){receiverSelect.value=currentReceiver;receiverSelect.disabled=true}else{receiverSelect.disabled=false;receiverSelect.value=""}});
  document.querySelector("#property-renewal-form").onsubmit=event=>{event.preventDefault();const form=new FormData(event.currentTarget);const documentNumber=String(form.get("documentNumber")||"").trim().toUpperCase();const employee=sameReceiver?.checked?currentReceiver:String(form.get("employee")||"");const duplicate=propertyUnits.some((unit,unitIndex)=>unitIndex!==index&&String(semi?unit.icsNumber:unit.parNumber).toUpperCase()===documentNumber);if(duplicate){document.querySelector("#renewal-error").textContent=`${documentLabel} number already exists.`;return}const previous={date:localISODate(),receiver:property.employee||"",documentNumber:oldDocument||"",qrVersion:Number(property.qrVersion||1)};property.renewalHistory=[...(property.renewalHistory||[]),previous];property.employee=employee;property.position=employeePosition(employee);if(semi)property.icsNumber=documentNumber;else property.parNumber=documentNumber;property.qrVersion=Number(property.qrVersion||1)+1;property.lastRenewalIsoDate=localISODate();property.renewalDueIsoDate=addYearsToIso(property.lastRenewalIsoDate,3);close();showToast(`${documentLabel} renewed. A new QR version is ready for printing.`);render("Dashboard")};
}
...
function bindEnhanced(){
  ...
  document.querySelectorAll("[data-renew-property]").forEach(button=>button.onclick=()=>openPropertyRenewal(+button.dataset.renewProperty));
  document.querySelectorAll("[data-sticker-field]").forEach(control=>control.addEventListener("change",event=>{stickerFields[event.target.dataset.stickerField]=event.target.checked;render("Inventory Sticker")}));
  document.querySelectorAll("[data-sticker-setting]").forEach(control=>control.addEventListener("change",event=>{stickerSettings[event.target.dataset.stickerSetting]=event.target.value;render("Inventory Sticker")}));
  document.querySelectorAll("[data-sticker-property]").forEach(control=>control.addEventListener("change",event=>{const key=event.target.dataset.stickerProperty;stickerSelectedKeys=event.target.checked?[...new Set([...stickerSelectedKeys,key])]:stickerSelectedKeys.filter(value=>value!==key);render("Inventory Sticker")}));
  document.querySelector("#sticker-search")?.addEventListener("input",event=>{stickerSearch=event.target.value;const query=stickerSearch.trim().toLowerCase();document.querySelectorAll(".sticker-property-option").forEach(row=>{row.hidden=Boolean(query&&!row.textContent.toLowerCase().includes(query))})});
  document.querySelector("#sticker-classification")?.addEventListener("change",event=>{stickerClassification=event.target.value;render("Inventory Sticker")});
  document.querySelector("#select-filtered-stickers")?.addEventListener("click",()=>{stickerSelectedKeys=[...new Set([...stickerSelectedKeys,...stickerFilteredEntries().map(entry=>entry.key)])];render("Inventory Sticker")});
  document.querySelector("#clear-stickers")?.addEventListener("click",()=>{stickerSelectedKeys=[];render("Inventory Sticker")});
  document.querySelector("#print-inventory-stickers")?.addEventListener("click",()=>window.print());
  document.querySelector("#new-po")?.addEventListener("click",openPO);
