drop function if exists issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text);
drop function if exists issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text, text);

create function issue_semi_expendable_property(
  p_property_id uuid,
  p_employee_id uuid,
...
begin
  ...
  if nullif(trim(p_inventory_item_number), '') is not null and trim(p_inventory_item_number) !~ '^\d{4}-\d{2}-\d{4}$' then
    raise exception 'Inventory Item No. must use the format 5030-04-0001';
  end if;
  if nullif(trim(p_inventory_item_number), '') is not null and exists (
    select 1 from property_units where inventory_item_number = trim(p_inventory_item_number) and id <> p_property_id
  ) then
  ...
  update property_units set
    ...
    inventory_item_number = case
      when nullif(trim(p_inventory_item_number), '') is not null then trim(p_inventory_item_number)
