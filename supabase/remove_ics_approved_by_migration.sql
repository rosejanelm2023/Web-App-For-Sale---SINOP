...
begin
  if nullif(trim(p_performed_by), '') is null then raise exception 'Signed-in user is required for the audit log'; end if;
  ...
  if (p_generate_ics or v_property.ics_number is not null) and coalesce(p_ics_number, '') !~ '^\d{4}-\d{2}-\d{4}$' then
    raise exception 'ICS No. must use the format YYYY-MM-0001';
  end if;
  ...
  update property_units set
    ics_number = case when p_generate_ics or ics_number is not null then nullif(trim(p_ics_number), '') else ics_number end,
    ...
    inventory_item_number = case when p_generate_ics then coalesce(inventory_item_number, v_inventory_prefix || '-' || to_char(v_issue_date, 'MM') || '-' || lpad(v_inventory_sequence::text, 4, '0')) else inventory_item_number end,
...
revoke all on function issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text) from public, anon;
grant execute on function issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text) to authenticated;
