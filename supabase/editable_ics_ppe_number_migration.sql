...
drop function if exists public.issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text, text);

create or replace function public.issue_semi_expendable_property(
  p_property_id uuid,
  p_employee_id uuid,
...
begin
  ...
  if coalesce(trim(p_ppe_number), '') !~ '^\d{4}-\d{2}-\d{2}-\d{4}$' then
    raise exception 'PPE No. must use YYYY-XX-XX-0001';
  end if;
  if exists (
    select 1 from property_units
...
revoke all on function public.issue_semi_expendable_property(uuid, uuid, uuid, boolean, text, text, text, text, text, text, text, text, text) from public, anon;
